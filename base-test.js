...
function checkBrowser() {
    ...
    var versionNum = info.version.substring(0, info.version.indexOf("."));
    if (info.browser === 'IE' && versionNum < 10) {
...
ga('create', 'UA-90877519-1', 'auto');
ga('send', 'pageview');


function yFind(selectors){
    return  document.querySelector(selectors);
}
function yStyle(obj,name) {

    if (window.getComputedStyle) {
        //Adapt to Chrome, Firefox, IE9 and above browsers
        return getComputedStyle(obj, null)[name];
    } else {
        //Adapt to IE8 browser
        return obj.currentStyle[name];

    }
}
window.onload = function () {
    var navDiv = yFind(".header_nav") ;
    var searchDiv =  yFind('.header_2');
    var categoryBtn =  yFind('#category_btn');


    var sticky = new Sticky(".header_nav");
    ...
    function resetSearchBar(isShow) {
        searchDiv.style.display = isShow?"flex":"none";
        var arrowIcon = categoryBtn.getElementsByTagName('i')[0];
        if (isShow){
            arrowIcon.className = 'icon ion-ios-arrow-up';
        } else{
            arrowIcon.className ='icon ion-ios-arrow-down';
        }
    }

    categoryBtn.click(function () {

        var isHidden = yStyle(searchDiv,"display") ==="none";
        if (isHidden){
            resetSearchBar(true);
        }else{
            resetSearchBar(false);
